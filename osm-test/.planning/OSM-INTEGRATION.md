...
- **권한 (2026-08-26 확정): 관리자는 전원, 일반 사용자는 같은 파견지 인원만.** 운영에서는 파견지 단위로
  참여 인원끼리만 서로 보이고, 관리자는 전체를 관리한다. 응답은 요청자의 역할·파견지로 서버에서 필터링한다
  — 전원 응답은 관리자 전용. 프론트 필터로 대신하지 않는다(응답에 섞이는 순간 노출이다)
- 테스트 구현: 계정에 `role`(ADMIN/USER) + `site_id`(파견지)를 두고 두 역할 모두 필터를 검증한다.
  실제 파견지 조직 데이터와의 연동은 기존 환경 이식 시점 몫
