...
Status: P0~P2 구현·검증 완료, 코드 존재
Last activity: 2026-08-26 — P0(tileserver-gl+curl 4종)·P1(React+MapLibre 지도)·P2(NGINX+eGov 스타일 백엔드+MariaDB, 권한 필터링 e2e 검증)까지 전부 완료. 컨테이너 4개(tileserver-gl, osm-test-mariadb, osm-test-nginx) + Tomcat + Vite dev 서버가 로컬에 기동 중
...
Stopped at: **P0~P2 구현·검증 완료.** tileserver-gl(:8081, `--public_url` 적용)·MariaDB 10.11(:3307)·
eGov 스타일 백엔드+Tomcat 9.0.78(:8082)·NGINX(:8888, 단일 오리진)·React+MapLibre 프론트(Vite :5173) 전부
로컬에 떠 있고 서로 연동 확인됨. 권한 필터링(관리자 전원/파견지 단위) e2e 실증 완료. 상세는
OSM-INTEGRATION.md 4절 P0·P1·P2 실행 결과, 삽질은 DEAD-ENDS.md
Next: P3 — PWA 캐싱, mkcert 인증서 + 실단말 필요 (사용자 진행). 로컬 컨테이너·프로세스를 계속
띄워둘지 정리할지 다음 세션에서 사용자와 확인
